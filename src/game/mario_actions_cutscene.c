    ...
    if (m->actionState == ACT_STATE_STAR_DANCE_CUTSCENE) {
        switch (++m->actionTimer) {
            ...
            case 80:
                ...
                if (!(m->actionArg & 1)) {
                    if (!(gMarioState->timeAttack)) level_trigger_warp(m, WARP_OP_STAR_EXIT);
                } else {
                    enable_time_stop();
...
s32 act_spawn_spin_airborne(struct MarioState *m) {
    ...
    */ // Makes Mario's spawn animation not seem off
...
s32 act_spawn_spin_landing(struct MarioState *m) {
    ...
    //if (is_anim_at_end(m)) { // Makes Mario not stop when landing
        load_level_init_text(0);
        set_mario_action(m, ACT_IDLE, 0);
    //}
    return FALSE;
